...
public abstract class BaseStreamDecoderTests {
    ...
    StreamDecoder.Builder builder;
    StreamListener addPar = sb -> { sb.insert(0, '(').append(')'); return false; };
    StreamListener addBraq = sb -> { sb.insert(0, '[').append(']'); return false; };
    ...
    public void generalTestWithHtml() throws Exception {
        ...
        writeBytesAndCheckResult(builder
                        .bind(bigPipePlaceholderMatcher, sb -> { sb.replace(0, sb.length(), realContent); return false; } )
                        .bind(closingBodyMatcher, sb -> { sb.insert(0, myScript); return false; } )
                        .bind(closingHeadMatcher, sb -> Boolean.FALSE ),
                originalHtml, expectedHtml);
    }

    abstract void writeBytesAndCheckResult(StreamDecoder.Builder builder, String text, String expected) throws Exception;
